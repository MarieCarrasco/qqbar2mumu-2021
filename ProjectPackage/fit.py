...
def CBdoubleExp(params,x):
  N, a, n, xb, sig, const1, slope1, mu1, const2, slope2, mu2 = params

  exp1 = const1*exp(slope1*x + mu1)
  exp2 = const2*exp(slope2*x + mu2)
  ...
  total += ((x-xb)/sig <= -a) * N * A * (B - (x-xb)/sig)**(-n)
  try:
    return total.real + exp1*exp2
  except:
    return total + exp1*exp2
  return total + exp1*exp2

def CBPow(params,x):
  N, a, n, xb, sig, base, exponent = params
  x = x+0j # Prevent warnings...
  if a < 0:
    a = -a
  if n < 0:
  ...
  total += ((x-xb)/sig <= -a) * N * A * (B - (x-xb)/sig)**(-n)
  try:
    return total.real + base*(x**exponent)
  except:
    return total + base*(x**exponent)
  return total + base*(x**exponent)
...
def get_default_params(xdata, ydata, func,xCB=None,yCB=None):
  ...
  elif func == CBdoubleExp:

    N = yCB.max()
    xb = xCB[yCB.argmax()]
    sigma = sum(yCB > yCB.mean()*1.8)*1. / len(xCB) * (xCB.max() - xCB.min())
    n = 2.
    a = .5

    xdatainf = xdata[ak.where(xdata < xCB[0])]
    xdatasup = xdata[ak.where(xdata > xCB[-1])]
    ydatainf = ydata[ak.where(xdata < xCB[0])]
    ydatasup = ydata[ak.where(xdata > xCB[-1])]

    (xdoubleExp,ydoubleExp) = (np.concatenate([xdatainf,xdatasup]),np.concatenate([ydatainf,ydatasup]))

    for i in range(21):
      try:
        args = [range(i), 0]
        result = double_exp(*args)
      except ValueError:
        pass
      else:
        break
    paramsdoubleExp = zeros(i) + xdoubleExp[len(xdoubleExp)/2]

    return array([N, a, n, xb, sigma,paramsdoubleExp])
  else:
...
def fit(func, x,y, default_pars=None, data_range=None, we=None, verbose=False, itmax=200,CB_range=None):
  ...
  IN: (func, x, y, verbose, itmax)
     ...
     data_range   - Fit a subrange of (x,y). Provide a tuple of the form
                    (x_min, x_max).
     CB_range   - range for the crystal ball fit (xmin,xmax)
  ...
  if CB_range:
    yCB = y[logical_and(x > CB_range[0], x < CB_range[1])]
    xCB = x[logical_and(x > CB_range[0], x < CB_range[1])]
  ...
  if default_pars != None:
    ...
    beta0 = get_default_params(x,y,func,xCB,yCB)
  else:
